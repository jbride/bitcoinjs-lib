...
const bip360_js_1 = require('./bip360.cjs');
...
function p2mr(a, opts) {
  ...
  const _hashTree = lazy.value(() => {
    if (a.scriptTree) return (0, bip360_js_1.toHashTree)(a.scriptTree);
  ...
  lazy.prop(o, 'hash', () => {
    ...
    if (w && w.length > 1) {
      ...
      const leafHash = (0, bip360_js_1.tapleafHash)({
        output: script,
        version: leafVersion,
      });
      return (0, bip360_js_1.rootHashFromPath)(controlBlock, leafHash);
    }
    return null;
  });
  lazy.prop(o, 'pubkey', () => {
  ...
  lazy.prop(o, 'witness', () => {
    ...
    if (a.scriptTree) {
      ...
      if (hashTree && a.redeem && a.redeem.output) {
        ...
        const path = (0, bip360_js_1.findScriptPath)(hashTree, leafHash);
        if (!path) return;
  ...
  if (opts.validate) {
    ...
    if (a.redeem && a.redeem.output && a.scriptTree && hashTree) {
      ...
      if (!(0, bip360_js_1.findScriptPath)(hashTree, leafHash))
